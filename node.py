...
my_levels = {"Nitrate Level":1310,"Phosphate Level":1600}       # Node level or requirements

sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM, socket.IPPROTO_UDP) # seperate socket for UDP Multicasting
sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) # Providing socket parameters to define socket behaviour
...
mreq = struct.pack("4sl", socket.inet_aton(MCAST_GRP), socket.INADDR_ANY) # Message request needed for the mulitcast receiving
if mreq:
    print("2.MREQ Done")
...
while True:
    try:
        data = pickle.loads(sock.recv(1024))        # Receiving data and loading it as json/dictionary using pickle
        if data:
...
sender_sock.close()     #Closing sockets
sock.close()            